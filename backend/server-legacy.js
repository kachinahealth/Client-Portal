...
app.get('/api/users/:userId/clinical-assignment', authenticateToken, requireSupabase, async (req, res) => {
  try {
...
app.put('/api/users/:userId/clinical-assignment', authenticateToken, requireSupabase, async (req, res) => {
  try {
    const { userId } = req.params;
...
app.get('/api/clinical-trials', authenticateToken, async (req, res) => {
  try {
    ...
    if (supabase && supabaseAdmin && process.env.SUPABASE_URL && process.env.SUPABASE_ANON_KEY && process.env.SUPABASE_SERVICE_ROLE_KEY) {
      try {
        ...
        if (error) {
          ...
          console.log('📨 Query successful, found', data.length, 'clinical trials');
          ...
          console.log('No data from Supabase, returning empty trials array');
        }
      } catch (dbError) {
        console.error('Database operation failed:', dbError);
        console.log('Continuing with empty trials array');
      }
    } else {
      console.log('⚠️ Supabase not configured for clinical trials - check environment variables');
...
app.post('/api/clinical-trials', authenticateToken, requireSupabase, async (req, res) => {
  try {
    const userId = req.user.userId;
    ...
    // Get user role information from profiles table
    const { data: userProfile, error: profileError } = await supabase
      .from('profiles')
      .select('organization_id, role')
      .eq('id', userId)
